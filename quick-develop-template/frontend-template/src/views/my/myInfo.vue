<template>
  <div class="content">
            ...
            <template #header>
              <div class="card-header">
                <el-row style="margin-bottom: 0">
                  <el-col :span="16">
                    <el-avatar shape="square" :size="140" :src="user.userAvatar" />
                    <el-space alignment="flex-start" direction="vertical">
                      <span class="avatarName">{{ user.userName }}</span>
                      <span style="margin: 15px">{{ user.userProfile ? user.userProfile : "暂无个人简介" }}</span>
                    </el-space>
                  </el-col>
                  <el-col :span="8">
                    <span style="float: right">
                           <el-button size="large" @click="toEditInfo" plain type="primary">编辑个人资料</el-button>
                  </span>
                  </el-col>
                </el-row>
              </div>
    ...
    <!--  编辑信息对话框-->
    <el-dialog v-model="editFormVisible" title="编辑用户信息" style="width: 580px;" draggable>
      <el-form :model="currentUser">
        ...
        <el-form-item label="简介" :label-width="formLabelWidth">
          <el-input v-model="currentUser.userProfile" type="textarea" autocomplete="off" />
        </el-form-item>
        <!--      性别选项-->
...
const confirmEdit = async () => {
  const res = await UpdateMyInfo(currentUser.value);
  if (res.code === 0) {
